...
public class FinancialManager {
	ArrayList<Financial> accounts = new ArrayList();
	Scanner input;
	...
	public void addAccount() {
		...
		while (kind != 1 && kind != 2) {
			...
			kind = input.nextInt();
			if (kind == 1) {
				account = new Financial(FinancialKind.IBK기업은행);
				account.getUserInput(input);
				accounts.add(account);	
				break;
			}
			else if (kind == 2) {
				account = new 우리은행(FinancialKind.우리은행);
				account.getUserInput(input);
				accounts.add(account);
				break;
			}
			else if (kind == 3) {
				account = new KB국민은행(FinancialKind.KB국민은행);
				account.getUserInput(input);
				accounts.add(account);	
				break;
			}
	...
	public void editAccount() {
		...
		int accountId = input.nextInt();
		for (int i = 0; i < accounts.size(); i++) {
			Financial account = accounts.get(i);
...
//		System.out.print("Account ID:");
//		int accountId = input.nextInt();
		System.out.println("# of registered students: " + accounts.size());
		for (int i = 0; i < accounts.size(); i++) {
